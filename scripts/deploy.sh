...
ssh "$DEPLOY_HOST" "cd $DEPLOY_DIR && sudo git checkout -- nginx/default.conf && sudo git pull && sudo docker compose build web && sudo docker compose run --rm web python manage.py migrate --noinput && sudo docker compose up -d web && sudo cp -f nginx/default-ssl.conf nginx/default.conf && sudo docker compose up -d --force-recreate nginx && sudo docker compose exec -T nginx nginx -t"
